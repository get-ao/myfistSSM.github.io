...
<!-- 标签页组 -->
  ...
  <ul class="nav nav-tabs" role="tablist">
    <li role="presentation" class="active"><a href="#home" aria-controls="home" role="tab" data-toggle="tab">岗位申请</a></li>
  </ul>
    ...
    <br>
    	<div class="page-header" style="margin-top: -15px">
  			<table class="table table-striped">
  				...
  				<tbody>
  					<tr>
  						<td>${requestScope.sjob.studentName }</td>
  						<td>${requestScope.sjob.applyJob }</td>
  						<td>${requestScope.sjob.auditSituation }</td>
  						<td>
  						<button id="nav"  class="btn btn-info">查看其他岗位</button>
  						</td>
  					</tr>
  				</tbody>
  			</table>
		...
		<div >
			<table id="joblist" style="display: none;" class="table table-striped">
			<thead>
				<tr>
					<th>招聘岗位</th>
					<th>招聘要求</th>
			...
			<tbody>
				<c:forEach items="${requestScope.jobInfo}" var="job">
				<c:if test="${job.state=='已通过'}">
				<tr>
					<td>${job.job }</td>
					<td>${job.jobNeed }</td>
					<td>${job.handName }</td>
					<td>
					<a href="${pageContext.request.contextPath}/studentControl/changeJob?job=${job.job}" class="btn btn-success">申请</a>
					</td>
				</tr>
				</c:if>
				</c:forEach>
